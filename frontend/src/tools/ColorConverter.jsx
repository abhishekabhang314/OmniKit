...
export default function ColorConverter() {
  ...
  const handleAction = async () => {
    ...
    setCopiedKey(null)
    try {
      const res = await axios.post('/api/converters/color', { color: colorInput })
      setResult(res.data)
    } catch (err) {
          ...
          <div className="grid grid-cols-1 gap-3">
            {result && [
              ...
              <div key={fmt.key} className="flex flex-col gap-1">
                <label className="text-xs font-medium text-[var(--color-text-secondary)] uppercase">{fmt.label}</label>
                <div className="flex items-center justify-between gap-4 bg-[var(--color-bg-subtle)] p-3 rounded-lg border border-[var(--color-border)]">
                  ...
                  >
                    {copiedKey === fmt.key ? <Check size={18} className="text-[var(--color-success)]" /> : <Copy size={18} />}
                  </button>
                </div>
              </div>
            ))}
