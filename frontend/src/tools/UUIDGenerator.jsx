...
import { ToolSlider, ToolButton, ToolResult, FieldGroup } from '@/components/ui-kit'
...
export default function UUIDGenerator() {
  ...
  const handleAction = async () => {
    ...
    setCopiedAll(false)
    try {
      const res = await axios.post('/api/generators/uuid', { 
    ...
    <div className="flex flex-col gap-6">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
        {[
          ...
          <label key={opt.key} className="flex items-center gap-3 p-3 border border-[var(--color-border)] rounded-md hover:bg-[var(--color-bg-hover)] cursor-pointer transition-colors">
            <input 
              type="checkbox" 
      ...
      {error && <p className="text-sm text-[var(--color-error)] mt-[-10px]">{error}</p>}
      ...
      <ToolResult visible={results.length > 0}>
        <div className="flex flex-col gap-2">
          ...
          <div className="flex flex-col gap-3">
            {results.map((uuid, i) => (
              <div key={i} className="flex items-center justify-between gap-4 bg-[var(--color-bg-subtle)] p-3 rounded-lg border border-[var(--color-border)]">
                ...
                >
                  {copiedIndex === i ? <Check size={18} className="text-[var(--color-success)]" /> : <Copy size={18} />}
                </button>
              </div>
            ))}
          </div>
