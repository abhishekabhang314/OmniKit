...
import { ToolInput, ToolSelect, ToolButton, ToolResult, FieldGroup } from '@/components/ui-kit'
...
const CATEGORIES = {
  length: { units: ['meter', 'kilometer', 'mile', 'yard', 'foot', 'inch', 'centimeter', 'millimeter'] },
  weight: { units: ['kilogram', 'gram', 'pound', 'ounce', 'ton'] },
  temperature: { units: ['celsius', 'fahrenheit', 'kelvin'] },
  speed: { units: ['meter_per_second', 'kilometer_per_hour', 'mile_per_hour', 'knot'] },
  area: { units: ['square_meter', 'square_kilometer', 'square_mile', 'acre', 'hectare'] },
}

const CATEGORY_LABELS = {
  length: 'Length', weight: 'Weight', temperature: 'Temperature', speed: 'Speed', area: 'Area',
}
...
export default function UnitConverter() {
  ...
  const options = CATEGORIES[category].units.map(u => ({ value: u, label: UNIT_LABELS[u] || u }))
    ...
    <div className="flex flex-col gap-6">
      ...
      <div className="grid grid-cols-[1fr_auto_1fr] gap-3 items-end">
        ...
        <Button
          ...
          title="Swap units"
        >
          <ArrowLeftRight size={18} className="transition-transform group-hover:rotate-180" />
        </Button>
        <ToolSelect
          label="To"
          id="uc-to"
      ...
      <ToolResult visible={result !== null} className="primary-accent text-center relative pt-8 pb-6">
