...
export default function BMICalculator() {
  ...
  const handleAction = async () => {
    ...
    setResult(null)
    try {
      const res = await axios.post('/api/calculators/bmi', { 
      ...
      <ToolResult visible={!!result}>
        {result && (
          <div className="flex flex-col items-center justify-center p-6 bg-[var(--color-bg-subtle)] rounded-xl border border-[var(--color-border)] gap-2">
            <div className="text-sm text-[var(--color-text-secondary)] uppercase tracking-wider font-semibold">Your BMI</div>
            <div className="text-5xl font-black text-[var(--color-text-primary)]">
              {result.bmi}
            </div>
            <div className={`text-xl font-bold mt-2 ${getCategoryColor(result.category)}`}>
              {result.category}
            </div>
          </div>
        )}
